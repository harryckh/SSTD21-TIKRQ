...
/**
 * <h>DataGen</h> to generate the data
 * 
 * @author feng zijin
 *
 */
public class DataGen {
	...
	/**
	 * add additional door in two staircases to model as lifts 
	 */
	public void linkFloorByLift() {

		// -------------------------
		///
		/// for each floor
//		System.out.println("DataGenConstant.nFloor: " + DataGenConstant.nFloor);

		for (int i = 0; i < DataGenConstant.nFloor; i++) {
			Floor floor = IndoorSpace.iFloors.get(i);

			List<Integer> doors = floor.getmDoors();
//			int curDoorsSize = doors.size();/// need a tmp var to store size as it increase in the loop
			int curDoorsSize =2; ///only change first two staircase to elevator
//			System.out.println("doors.size():" + doors.size());
//			System.out.println(doors);
			for (int j = 0; j < curDoorsSize; j++) {
				// get reference from existing staircase doors
				Integer refDoorID = doors.get(j);
				Door refDoor = IndoorSpace.iDoors.get(refDoorID);
				int refPartID = refDoor.getmPartitions().get(0);
				Partition refPart = IndoorSpace.iPartitions.get(refPartID);
				// for each part in other floor

				for (int k = i + 1; k < DataGenConstant.nFloor; k++) {
					if (i == k)
						continue;
//					System.out.println("i:" + i + " j:" + j + " k:" + k);
					int otherFloorParID = refPartID + (k - i) * IndoorSpace.iNumberParsPerFloor;
					Partition otherFloorPar = IndoorSpace.iPartitions.get(otherFloorParID);

					// gen new door
					Door door = new Door(refDoor.getX(), refDoor.getY());
					door.setmType(DoorType.EXIT);
					door.setmFloor(k);
					door.setmID(IndoorSpace.iDoors.size());

					// fix the connection
					door.addPar(refPartID);
					door.addPar(otherFloorParID);
					refPart.addDoor(door.getmID());
					otherFloorPar.addDoor(door.getmID());
					floor.addDoor(door.getmID());
					IndoorSpace.iDoors.add(door);

//					System.out.println("generated new door: refDoorID: " + refDoorID + " newdoor: " + door.toString());
				}
			}
...
//				System.out.println("doorId:" + doorId);
				...
				/// the part in the below floor ?
				Partition downPar = IndoorSpace.iPartitions.get(downPartId);
				/// add the door to down part
				ArrayList<Integer> mDoors = downPar.getmDoors();
				mDoors.add(doorId);
				downPar.setmDoors(mDoors);

				/// add downPart to this door
				parts.add(downPartId);
				door.setmPartitions(parts);
//				System.out.println("getmType: " + door.getmType());
//				System.out.println("downPar: " + downPar.getmID() + " door.mpart:" + door.getmPartitions().toString());
			}
		}
	}
...
//				System.out.println("door generated with id = " + (newDoor.getmID() + 1) + " on floor = " + newDoor.getmFloor());
			...
			floor.addDoor(
					IndoorSpace.iDoors.get(IndoorSpace.iNumberDoorsPerFloor - 6 + floorIndex * curSizeDoor).getmID());
			IndoorSpace.iDoors.get(IndoorSpace.iNumberDoorsPerFloor - 6 + floorIndex * curSizeDoor)
					.addFloor(floorIndex - 1);
			IndoorSpace.iDoors.get(IndoorSpace.iNumberDoorsPerFloor - 6 + floorIndex * curSizeDoor)
					.addFloor(floorIndex);
...
//			System.out.println((IndoorSpace.iDoors.get(210 + floorIndex * curSizeDoor).getmID() == (210 + floorIndex * curSizeDoor)) + " " + (210 + floorIndex * curSizeDoor));
			...
			floor.addDoor(
					IndoorSpace.iDoors.get(IndoorSpace.iNumberDoorsPerFloor - 4 + floorIndex * curSizeDoor).getmID());
			IndoorSpace.iDoors.get(IndoorSpace.iNumberDoorsPerFloor - 4 + floorIndex * curSizeDoor)
					.addFloor(floorIndex - 1);
			IndoorSpace.iDoors.get(IndoorSpace.iNumberDoorsPerFloor - 4 + floorIndex * curSizeDoor)
					.addFloor(floorIndex);
			if (floorIndex < (floorNumber - 1))
...
//			System.out.println((IndoorSpace.iDoors.get(212 + floorIndex * curSizeDoor).getmID() == (212 + floorIndex * curSizeDoor)) + " " + (212 + floorIndex * curSizeDoor));
			...
			floor.addDoor(
					IndoorSpace.iDoors.get(IndoorSpace.iNumberDoorsPerFloor - 3 + floorIndex * curSizeDoor).getmID());
			IndoorSpace.iDoors.get(IndoorSpace.iNumberDoorsPerFloor - 3 + floorIndex * curSizeDoor)
					.addFloor(floorIndex - 1);
			IndoorSpace.iDoors.get(IndoorSpace.iNumberDoorsPerFloor - 3 + floorIndex * curSizeDoor)
					.addFloor(floorIndex);
			if (floorIndex < (floorNumber - 1))
...
//			System.out.println((IndoorSpace.iDoors.get(213 + floorIndex * curSizeDoor).getmID() == (213 + floorIndex * curSizeDoor)) + " " + (213 + floorIndex * curSizeDoor));
			...
			floor.addDoor(
					IndoorSpace.iDoors.get(IndoorSpace.iNumberDoorsPerFloor - 2 + floorIndex * curSizeDoor).getmID());
			IndoorSpace.iDoors.get(IndoorSpace.iNumberDoorsPerFloor - 2 + floorIndex * curSizeDoor)
					.addFloor(floorIndex - 1);
			IndoorSpace.iDoors.get(IndoorSpace.iNumberDoorsPerFloor - 2 + floorIndex * curSizeDoor)
					.addFloor(floorIndex);
			if (floorIndex < (floorNumber - 1))
			...
			while (itrPar.hasNext()) {
				...
				while (itrDoor.hasNext()) {
					...
					if (curPar.testDoor(curDoor) == true) {
						...
						// System.out.println((curPar.getmID() + 1) + " has door " + (curDoor.getmID() +
						// 1));
					}
				}
			}
			...
			while (itrPar.hasNext()) {
				...
				while (itrDoor.hasNext()) {
					...
					if (curPar.testDoor(curDoor) == true) {
						...
						// System.out.println((curPar.getmID() + 1) + " has door " + (curDoor.getmID() +
						// 1));
					} else if ((curPar.getmType() == RoomType.HALLWAY)
							&& ((curDoor.getX() <= curPar.getX2() && curDoor.getX() >= curPar.getX1())
									|| (curDoor.getX() >= curPar.getX2() && curDoor.getX() <= curPar.getX1()))
							&& ((curDoor.getY() <= curPar.getY2() && curDoor.getY() >= curPar.getY1())
									|| (curDoor.getY() >= curPar.getY2() && curDoor.getY() <= curPar.getY1()))) {
						if (curDoor.getmID() % 42 == 13 || curDoor.getmID() % 42 == 16 || curDoor.getmID() % 42 == 19
								|| curDoor.getmID() % 42 == 23 || curDoor.getmID() % 42 == 26
								|| curDoor.getmID() % 42 == 29 || curDoor.getmID() % 42 == 38
								|| curDoor.getmID() % 42 == 40)
							continue;
						curPar.addDoor(curDoor.getmID());
						curDoor.addPar(curPar.getmID());
						// System.out.println((curPar.getmID() + 1) + " has door " + (curDoor.getmID() +
...
//						+ IndoorSpace.iDoors.get(IndoorSpace.iDoors.size() - 1).getY() + ")");
			...
			// on the right part
			if ((partitionNo >= 7 && partitionNo <= 11) || (partitionNo == 15) || (partitionNo == 19)
					|| (partitionNo == 22) || (partitionNo >= 25 && partitionNo <= 27) || (partitionNo == 31)) {
//				System.out.print("right part ");
...
//						+ IndoorSpace.iDoors.get(IndoorSpace.iDoors.size() - 1).getY() + ")");
			...
			// on the bottom part
			if ((partitionNo >= 2 && partitionNo <= 6) || (partitionNo == 18) || (partitionNo == 20)
					|| (partitionNo >= 24 && partitionNo <= 25) || (partitionNo == 29) || (partitionNo == 32)) {
//				System.out.print("bottom part ");
...
//				twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size() - 1)));
	...
	private void genStaircase() {
		System.out.println("==> staircases & elevators");

//		IndoorSpace.iPartitions.add(new Partition(648.0, 720.0, 144.0, 288.0, RoomType.STAIRCASE));
		IndoorSpace.iPartitions.add(new Partition(648.0, 720.0, 144.0, 288.0, RoomType.ELEVATOR));
//		System.out.println("Partition " + (DataGenConstant.mID_Par-1) + " created "
//				+ twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size() - 1)));

//		IndoorSpace.iPartitions.add(new Partition(648.0, 720.0, 1080.0, 1224.0, RoomType.STAIRCASE));
		IndoorSpace.iPartitions.add(new Partition(648.0, 720.0, 1080.0, 1224.0, RoomType.ELEVATOR));
//		System.out.println("Partition " + (DataGenConstant.mID_Par-1) + " created "
//				+ twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size() - 1)));
...
//				+ twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size() - 1)));
	...
	private void genOtherDoor() {
		...
		while (itr.hasNext()) {
			...
			// on the upper part
			if ((partitionNo >= 129 && partitionNo <= 133) || (partitionNo >= 138 && partitionNo <= 139)) {
//				System.out.print("upper part ");
...
//						+ IndoorSpace.iDoors.get(IndoorSpace.iDoors.size() - 1).getY() + ")");
	...
	private void setDoorType() {
		int[] exitDoors = { IndoorSpace.iDoors.size() - 6, IndoorSpace.iDoors.size() - 4, IndoorSpace.iDoors.size() - 3,
				IndoorSpace.iDoors.size() - 2 };
...
//		System.out.println("part 1");
		for (int i = 0; i < 5; i++) {
			IndoorSpace.iPartitions
					.add(new Partition(288.0 + 72 * i, 288.0 + 72 * (i + 1), 144.0, 288.0, RoomType.STORE));
//			System.out.println("Partition " + DataGenConstant.mID_Par + " created " + 
//					twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size() - 1)));
		}
...
//		System.out.println("part 2");
		for (int i = 0; i < 5; i++) {
			IndoorSpace.iPartitions
					.add(new Partition(144.0, 288.0, 288.0 + 72 * i, 288.0 + 72 * (i + 1), RoomType.STORE));
//			System.out.println("Partition " + DataGenConstant.mID_Par + " created " + 
//					twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size() - 1)));
		}
...
//		System.out.println("part 3");
		for (int i = 0; i < 4; i++) {
			IndoorSpace.iPartitions
					.add(new Partition(360.0 + 72 * i, 360.0 + 72 * (i + 1), 360.0, 432.0, RoomType.STORE));
//			System.out.println("Partition " + DataGenConstant.mID_Par + " created " + 
//					twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size() - 1)));
		}
...
//		System.out.println("part 4");
		for (int i = 0; i < 3; i++) {
			IndoorSpace.iPartitions
					.add(new Partition(360.0, 432.0, 432.0 + 72 * i, 432.0 + 72 * (i + 1), RoomType.STORE));
//			System.out.println("Partition " + DataGenConstant.mID_Par + " created " + 
//					twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size() - 1)));
		}
...
//				twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size() - 1)));
		...
		for (int i = 0; i < 2; i++) {
			IndoorSpace.iPartitions
					.add(new Partition(576.0 + 36 * i, 576.0 + 36 * (i + 1), 522.0, 576.0, RoomType.STORE));
//			System.out.println("Partition " + DataGenConstant.mID_Par + " created " + 
//					twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size() - 1)));
		}
		...
		for (int i = 0; i < 2; i++) {
			IndoorSpace.iPartitions
					.add(new Partition(522.0, 576.0, 576.0 + 36 * i, 576.0 + 36 * (i + 1), RoomType.STORE));
//			System.out.println("Partition " + DataGenConstant.mID_Par + " created " + 
//					twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size() - 1)));
		}
		...
		if (divideType == 1 || divideType == 2) {
			...
			// System.out.println("Partition " + DataGenConstant.mID_Par + " created " +
			// twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size()
			// - 1)));
			...
			if (divideType == 1) {
				for (int i = 0; i < 2; i++) {
					IndoorSpace.iPartitions
							.add(new Partition(360.0 + 144 * i, 360.0 + 144 * (i + 1), 288, 360.0, RoomType.HALLWAY));
					// System.out.println("Partition " + DataGenConstant.mID_Par + " created " +
					// twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size()
					// - 1)));
				}
				...
				for (int i = 0; i < 2; i++) {
					IndoorSpace.iPartitions
							.add(new Partition(288.0, 360.0, 360 + 144 * i, 360.0 + 144 * (i + 1), RoomType.HALLWAY));
					// System.out.println("Partition " + DataGenConstant.mID_Par + " created " +
					// twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size()
					// - 1)));
				}
			...
			if (divideType == 2) {
				for (int i = 0; i < 4; i++) {
					IndoorSpace.iPartitions
							.add(new Partition(360.0 + 72 * i, 360.0 + 72 * (i + 1), 288, 360.0, RoomType.HALLWAY));
					// System.out.println("Partition " + DataGenConstant.mID_Par + " created " +
					// twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size()
					// - 1)));
				}
				...
				for (int i = 0; i < 4; i++) {
					IndoorSpace.iPartitions
							.add(new Partition(288.0, 360.0, 360 + 72 * i, 360.0 + 72 * (i + 1), RoomType.HALLWAY));
					// System.out.println("Partition " + DataGenConstant.mID_Par + " created " +
					// twoEndPointToString(IndoorSpace.iPartitions.get(IndoorSpace.iPartitions.size()
					// - 1)));
				}
...
//							+ IndoorSpace.iDoors.get(IndoorSpace.iDoors.size() - 1).getY() + ")");
				...
				if ((partitionNo >= 2 && partitionNo <= 6) || (partitionNo >= 13 && partitionNo <= 14)
						|| (partitionNo == 18) || (partitionNo == 20) || (partitionNo >= 24 && partitionNo <= 25)
						|| (partitionNo >= 30 && partitionNo <= 33) || (partitionNo == 36)) {
					// System.out.print("bottom part ");
...
//							+ IndoorSpace.iDoors.get(IndoorSpace.iDoors.size() - 1).getY() + ")");
	...
	private void genOtherDoorforDivide() {
			...
			while (itr.hasNext()) {
				...
				if ((partitionNo >= 97 && partitionNo <= 99)) {
					...
					if (IndoorSpace.iDoors.contains(aDoor))
						;
					IndoorSpace.iDoors.add(aDoor);
					// System.out.println("partition " + partitionNo + " has door " +
					// (IndoorSpace.iDoors.size()) + " at ("
					// + IndoorSpace.iDoors.get(IndoorSpace.iDoors.size() - 1).getX() + ", "
					// + IndoorSpace.iDoors.get(IndoorSpace.iDoors.size() - 1).getY() + ")");
				}
		...
		if (DataGenConstant.divisionType == 2) {
			...
			while (itr.hasNext()) {
				...
				// on the upper part
				if ((partitionNo >= 145 && partitionNo <= 149) || (partitionNo >= 154 && partitionNo <= 155)) {
					// System.out.print("upper part ");
	...
	private void parReflection(int axis, int pivot) {
		...
		for (int i = 0; i < curSize; i++) {
			...
			IndoorSpace.iPartitions
					.add(new Partition(Math.min(xa, xb), Math.max(xa, xb), Math.min(ya, yb), Math.max(ya, yb), type));
...
//		}
		...
		private void paintDoors(Graphics g) {
			...
			while (itr2.hasNext()) {
				...
				if (curDoor.getmID() >= IndoorSpace.iNumberDoorsPerFloor)
					break;
				g.fillOval((int) (curDoor.getX() * DataGenConstant.zoomLevel),
						(int) (curDoor.getY() * DataGenConstant.zoomLevel), 6, 6);
