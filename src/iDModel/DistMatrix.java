...
/**
 * <h>DistMatrix</h> represent the exit to exit (exit2exit) distance
 * 
...
public class DistMatrix {
	...
	private void createMatrix() throws IOException {
		if (DataGenConstant.divisionType == 0) {
			...
			// System.out.println(partition.getmID() + " has door = " +
			// IndoorSpace.iPartitions.get(partition.getmID()).getmDoors().size());
			// System.out.println("D2dHashMap size = " + partition.getD2dHashMap().size() +
			...
			if (partition.getmType() == RoomType.HALLWAY) {
				...
				for (int i = 0; i < doors.size(); i++) {
					for (int j = i + 1; j < doors.size(); j++) {
						...
						double distance = (double) hallwayDistMatrix
								.get(door1.getX() + "." + door1.getY() + "-" + door2.getX() + "." + door2.getY());
						matrix[i][j] = distance;
						matrix[j][i] = distance;
					}
				}
			...
			if (isPartition) {
				...
				// System.out.println(Functions.print2Ddoublearray(matrix, 0, matrix.length, 0,
				// matrix[0].length));
				...
				// System.out.println(floor.getmID() + " has door = " +
				// IndoorSpace.iFloors.get(floor.getmID()).getmDoors().size());
				// System.out.println("D2dHashMap size = " + floor.getD2dHashMap().size() + "
				// matrix size = " + matrix.length + " * "
				// + "" + matrix[0].length);
	...
	public double getDistance(int d1, int d2) {
		...
		if (index1 < 0 || index2 < 0 || index1 > matrix.length || index2 > matrix[0].length)
			return -1;
